...
const makeFallbackBody = (j4xId) => `<div id="${j4xId}"></div>`;
...
exports.get = function(req) {
    ...
    j4xId = ((j4xId || "") + "").replace(/\s+/g, "");
    if (j4xId === "") {
        j4xId = "j4x";
    }
    j4xId += "_" + Math.floor(Math.random() * 99999999); // TODO: Better make this determined by component path?
    ...
    // If the body is missing a target placeholder element: Make one and insert it right before the closing tag.
        ...
        const j4xPattern = new RegExp("<[^>]+\\s+id\\s*=\\s*[\"']" + j4xId + "[\"']", 'i');
        log.info(JSON.stringify({j4xId, j4xPattern}, null, 2));

        if (!body.match(j4xPattern)) {
            ...
            body = body.slice(0, endPos) + makeFallbackBody(j4xId) + body.slice(endPos);
     ...
     *              akkurat denne thymeleaf-ID'en et eller annet sted: <div id="${j4xId}"></div>.
     *          -Hvis ikke noe view er sendt inn, rendres kun: <div id="${j4xId}"></div>
     *              
     *          -Hvis j4xId er tom, genereres en unik id: j4x_[hash/uid]
     *          -Hvis j4xId ikke er tom, legges en unik hash/uid etter, som postfix.
     ...
     *                      React.render(toppkomponent, document.getElementById(j4xId))
     * 
